    ...
    <div class="user-profile grid grid-cols-12 bg-gray-50">
        <div class="py-10 lg:pl-2 border-r h-screen md:block hidden" :class="showTitles === false ? 'lg:col-span-2': 'col-span-1'">
            <div class="sidebar-logo">
                ...
                <svg aria-label="Instagram" class="mx-auto lg:hidden _ab6-" color="#262626" fill="#262626" height="24" role="img" viewBox="0 0 24 24" width="24" v-show="(showTitles === false)">
                    <path d="M12 2.982c2.937 0 3.285.011 4.445.064a6.087 6.087 0 0 1 2.042.379 3.408 3.408 0 0 1 1.265.823 3.408 3.408 0 0 1 .823 1.265 6.087 6.087 0 0 1 .379 2.042c.053 1.16.064 1.508.064 4.445s-.011 3.285-.064 4.445a6.087 6.087 0 0 1-.379 2.042 3.643 3.643 0 0 1-2.088 2.088 6.087 6.087 0 0 1-2.042.379c-1.16.053-1.508.064-4.445.064s-3.285-.011-4.445-.064a6.087 6.087 0 0 1-2.043-.379 3.408 3.408 0 0 1-1.264-.823 3.408 3.408 0 0 1-.823-1.265 6.087 6.087 0 0 1-.379-2.042c-.053-1.16-.064-1.508-.064-4.445s.011-3.285.064-4.445a6.087 6.087 0 0 1 .379-2.042 3.408 3.408 0 0 1 .823-1.265 3.408 3.408 0 0 1 1.265-.823 6.087 6.087 0 0 1 2.042-.379c1.16-.053 1.508-.064 4.445-.064M12 1c-2.987 0-3.362.013-4.535.066a8.074 8.074 0 0 0-2.67.511 5.392 5.392 0 0 0-1.949 1.27 5.392 5.392 0 0 0-1.269 1.948 8.074 8.074 0 0 0-.51 2.67C1.012 8.638 1 9.013 1 12s.013 3.362.066 4.535a8.074 8.074 0 0 0 .511 2.67 5.392 5.392 0 0 0 1.27 1.949 5.392 5.392 0 0 0 1.948 1.269 8.074 8.074 0 0 0 2.67.51C8.638 22.988 9.013 23 12 23s3.362-.013 4.535-.066a8.074 8.074 0 0 0 2.67-.511 5.625 5.625 0 0 0 3.218-3.218 8.074 8.074 0 0 0 .51-2.67C22.988 15.362 23 14.987 23 12s-.013-3.362-.066-4.535a8.074 8.074 0 0 0-.511-2.67 5.392 5.392 0 0 0-1.27-1.949 5.392 5.392 0 0 0-1.948-1.269 8.074 8.074 0 0 0-2.67-.51C15.362 1.012 14.987 1 12 1Zm0 5.351A5.649 5.649 0 1 0 17.649 12 5.649 5.649 0 0 0 12 6.351Zm0 9.316A3.667 3.667 0 1 1 15.667 12 3.667 3.667 0 0 1 12 15.667Zm5.872-10.859a1.32 1.32 0 1 0 1.32 1.32 1.32 1.32 0 0 0-1.32-1.32Z"></path>
                </svg>                                
            </div>
            <div class="list mt-10" :class="showTitles === true ? 'flex justify-center': ''">
                <ul>
                    <li 
                        class="my-4 flex items-center cursor-pointer mx-3 px-2 py-2 hover:rounded-full hover:bg-slate-100 search"
                        :class="tab === index ? 'text-orange-800' : ''"
                        v-for="(item, index) in sidebarList"
                        :key="index"
                        @click="changeTab(index, item.title)"
                    >
                        <img :src="user.picture" class="w-6 h-6 rounded-full" alt="useravatar" v-if="item.title === 'Profile'">
                        <button 
                            v-html="item.svg" 
                            class="target"
                        ></button>
                        <Transition name="slide-fade">
                            <span class="ml-8 hidden lg:block" v-show="!showTitles">
                                {{ item.title }}
                            </span>
                        </Transition>
                    </li>                                                                       
                ...
                <div class="w-60 h-58 fixed bottom-20 left-6" v-show="showMore">
                    <ul>
                        ...
                        <li 
                            class="flex items-center justify-between py-2 px-3 my-[0.125rem] rounded border border-gray-200 shadow bg-white"
                        >
                ...
                <div class="fixed bottom-10 left-6">
                    <button class="flex items-center" @click="showMore = !showMore">
                        <svg aria-label="Settings" class="_ab6-" color="#262626" fill="#262626" height="24" role="img" viewBox="0 0 24 24" width="24"><path d="M3.5 6.5h17a1.5 1.5 0 0 0 0-3h-17a1.5 1.5 0 0 0 0 3Zm17 4h-17a1.5 1.5 0 0 0 0 3h17a1.5 1.5 0 0 0 0-3Zm0 7h-17a1.5 1.5 0 0 0 0 3h17a1.5 1.5 0 0 0 0-3Z"></path></svg>
                        <span class="ml-4 font-bold">More</span>
                    </button>
                </div>
            </div>
        </div>
        <Transition name="slide-fade" ref="el">
            <div class="w-80 z-0 bg-gray-50 border rounded-r-xl shadow-md" v-show="showTitles">
                <div class="text-center mx-2">
                    ...
                    <div class="input_wrapper border-b py-3">
                        <input type="text" class="w-full rounded-md outline-0 bg-gray-200 shadow my-4 py-2 px-3" placeholder="Search" >
                    </div>
                    <ul>
                        <li v-for="(suggest, index) in 3" :key="index" class="hover:bg-slate-100 py-2 my-3 cursor-pointer px-5">
                            <div class="flex items-center justify-between">
                                ...
                                <div>
                                    <button class="text-gray-500 border-0 rounded-0 outline-0 text-bold">
        ...
        <div class="md:col-span-10 col-span-12">
            <component :is="pages[tab]"></component>
        </div>
    </div>
</template>
...
export default {
    ...
    setup () {
        ...
        const pages = reactive([ 'UserHome', 'UserHome', 'UserExplore', 'UserMessages', 'UserHome', 'NewPost', 'UserProfileInfo' ])
        ...
        const sideBarMore = reactive([
            { title: 'Settings', svg: '<svg aria-label="Settings" class="_ab6-" color="#262626" fill="#262626" height="24" role="img" viewBox="0 0 24 24" width="24"><circle cx="12" cy="12" fill="none" r="8.635" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></circle><path d="M14.232 3.656a1.269 1.269 0 0 1-.796-.66L12.93 2h-1.86l-.505.996a1.269 1.269 0 0 1-.796.66m-.001 16.688a1.269 1.269 0 0 1 .796.66l.505.996h1.862l.505-.996a1.269 1.269 0 0 1 .796-.66M3.656 9.768a1.269 1.269 0 0 1-.66.796L2 11.07v1.862l.996.505a1.269 1.269 0 0 1 .66.796m16.688-.001a1.269 1.269 0 0 1 .66-.796L22 12.93v-1.86l-.996-.505a1.269 1.269 0 0 1-.66-.796M7.678 4.522a1.269 1.269 0 0 1-1.03.096l-1.06-.348L4.27 5.587l.348 1.062a1.269 1.269 0 0 1-.096 1.03m11.8 11.799a1.269 1.269 0 0 1 1.03-.096l1.06.348 1.318-1.317-.348-1.062a1.269 1.269 0 0 1 .096-1.03m-14.956.001a1.269 1.269 0 0 1 .096 1.03l-.348 1.06 1.317 1.318 1.062-.348a1.269 1.269 0 0 1 1.03.096m11.799-11.8a1.269 1.269 0 0 1-.096-1.03l.348-1.06-1.317-1.318-1.062.348a1.269 1.269 0 0 1-1.03-.096" fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="2"></path></svg>' },
            { title: 'Saved', svg: '<svg aria-label="Saved" class="_ab6-" color="#262626" fill="#262626" height="24" role="img" viewBox="0 0 24 24" width="24"><polygon fill="none" points="20 21 12 13.44 4 21 4 3 20 3 20 21" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></polygon></svg>' },
